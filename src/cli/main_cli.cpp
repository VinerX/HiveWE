		...
		emit({{"ok", true},
			  ...
			  {"notes", json::array({
				   ...
				   "When Warcraft III data (CASC) is unavailable, the map-only paths of show-building / list-race-objects / list-all-races resolve base-object names from data/wc3_name_fallback.tsv.",
					   ...
					   "TERRAIN TRAVERSABILITY (pathing-islands / pathing-path): reads war3map.wpm, the engine's ground-truth movement grid. Use pathing-islands to find isolated landmasses (each 'island' is a connected walkable region) so you can decide where portals/transports/regions are needed; use pathing-path to test whether B is reachable from A for a movement type, optionally adding hypothetical --portals to see if they connect two islands (used_portal / same_island tell you). Movement types: foot (dry land), water (naval), amphibious (land+water), fly (ignores ground). NOTE: uses only the static pathing map (cliffs/water); placed trees/gates (dynamic pathing) are not yet applied, so a route through a forest may read as open.",
