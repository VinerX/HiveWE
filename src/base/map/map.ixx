...
export class Map: public QObject {
  ...
  public:
	...
	// data. Delegates to the free function in ObjectData.
	ObjectMergePlan compute_object_merge() {
		auto tables = object_tables();

		std::array<ShadowData, 7> mine;
		for (int i = 0; i < 7; i++) {
			mine[i] = tables[i].slk->shadow_data;
		}

		std::array<ObjectMergeTableDesc, 7> descs;
		for (int i = 0; i < 7; i++) {
			descs[i] = { tables[i].name, tables[i].slk, tables[i].meta,
						 tables[i].optional_ints, tables[i].files };
		}

		auto theirs = load_theirs_shadows(descs);
		return ::compute_object_merge(shadow_at_load, mine, theirs, descs);
	}
